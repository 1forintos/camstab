...
#                else:
#                	ledOn = True
#                wiringpi.digitalWrite(5, ledOn)
#                wiringpi.digitalWrite(pwm_pin_vertical, ledOn)
#                time_start = time.time()
#	pwmSignal = pwmSignal + 0.02
#        if pwmSignal > 300:
#        	pwmSignal = 0
#	print("PWM YO", pwmSignal)		
#       wiringpi.pwmWrite(pwm_pin_horizontal, int(pwmSignal))
...
#	speedRange = maxSpeed - minSpeed
	...
	if distanceX > maxX * (0.01 * thresholdX):
		if relativePosX < 0:	# move left
			# relative speed
			#moveX = -1 * (distanceX / maxDistanceX) * maxMoveX
